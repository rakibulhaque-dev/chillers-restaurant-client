...
const SingleRecipe = ({ data, chefData }) => {
    ...
    const handleFavorite = (name, img, price, chef_name) => {
        const obj = {name, img, price, chef_name}
    ...
    return (
        <div className='container p-6 mx-auto mb-5 border rounded-md shadow-lg'>
            ...
            <button onClick={()=>handleFavorite(recipe_name, img, price, chef_name)} className='flex gap-4 mt-2 btn btn-outline'><FaRegHeart></FaRegHeart> <small>Add Favorite</small></button>
        </div>
    );
};
